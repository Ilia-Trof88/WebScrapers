...
class TassScraper(BaseScraper):
    ...
    def _get_page_news(self,
                      ...
                      tass_headers: dict,
                      news_limit: int = 30,
                      search_after: str = None) -> list[dict] | None:
        '''
        ...
        except Exception as e: # Most likely the KeyError will be encountered, yet connection troubles are also possible
    ...
    def _parse_single_article(self,
        ...
        def get_article_text(news_dictionary: dict) -> str:
            ...
            if 'items' in news_dictionary['content_blocks'][0]: # Если в первом 'items' есть текст - значит есть текст новости.

                for dictionary in news_dictionary['content_blocks']:

                    try:

                        news_text+=dictionary['items'][0]['text'] + ' ' # Проверяем наличие текста в каждом словаре.
        ...
        try:
            ...
            print(f'An error occured while trying to scrape news article!\nError text: {e}')
            return None

    def scrape_rubric(self,
                      search_api_url: str,
                      tass_headers: dict,
                      tass_content_url: str,
        ...
        while fail_counter != max_retry_attemps:
            ...
            try:
                ...
                search_after = news_list[-1]['timestamp'] # Обновляем исходную дату, от которой идет поиск новостей (по убыванию)
