...
class Scrapper:
    ...
    def __init__(self,
        ...
        if category not in allowed_categories:
            ...
            raise KeyError(f'Введенная тематика не поддерживается! Поддерживаются следующие тематики: {', '.join(allowed_categories)}')
...
# Parsing news body
    ...
    def parse_news_article(self,
        ...
        try:
            ...
            date_class = page_content.find('time', class_= 'entry-date updated td-module-date')

            publish_date = date_class['datetime']

            tags = ', '.join([tag.get_text(strip = True) for tag in tags_class.find_all('a')])

            content_div = page_content.find('div', class_ = "td_block_wrap tdb_single_content tdi_73 td-pb-border-top td_block_template_1 td-post-content tagdiv-type")

            artcile_text = ' '.join([x.get_text(strip = True) for x in content_div.find_all('p')])
    ...
    def scrape_news(self,
        ...
        while len(news_list) < n_news:
            ...
            for url in all_page_urls:
                ...
                if 'Unknown' not in news_data.values(): # Если есть одно значение Unknown, значит все остальные также Unknown - не добавляем такие элементы в общий список

                    news_list.append(news_data)
            
            iteration_counter+=1 # Увеличиваем для корректного перехода по страницам сайта.
