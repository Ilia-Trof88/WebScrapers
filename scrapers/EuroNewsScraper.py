...
class EuroNewsScraper(BaseScraper):
    ...
    def _get_page_news(self,
        ...
        news_listing_el = page_content.find('div', class_='h-grid h-grid-body b-listing__body')
    ...
    def _parse_headline(self,
                       bs_object: BeautifulSoup) -> str | None:
        '''
    ...
    def _parse_author(self,
        ...
        concrete_author = concrete_author.text.strip() if concrete_author is not None else None
    ...
    def _parse_single_article(self,
        ...
        if '/video/' in article_url: # Примитивная проверка, на EuroNews есть новостные статьи, в который представлено только видео, осознанно пропускаем такие статьи
    ...
    def _get_main_paginator(self,
                            page_url: str) -> BeautifulSoup:
        '''
    ...
    def _get_total_num_pages(self,
        ...
        paginator_buttons = main_paginator.find_all(['a', 'span'], class_='c-paginator__button')
    ...
    def _get_current_page_num(self,
        ...
        current_page_num = main_paginator.find('span', class_='c-paginator__button c-current-page').text
    ...
    def parse_rubric(self,
                      start_url: str,
                      num_collected_news: int = 100) -> list[dict]:
        '''
        ...
        rubric_num_pages = self._get_total_num_pages(page_url = start_url) # Сколько страниц в рубрике

        collected_news = 0

        while collected_news < num_collected_news:

            page_news = self._get_page_news(page_url = start_url) # Получаем все новости со страницы
            ...
            print(f'Успешно собраны новости со страницы {page_number} из {rubric_num_pages}')
