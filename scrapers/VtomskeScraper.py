...
class NewsParser:
    ...
    def parse_single_article(self,
        ...
        search_pattern = re.compile(r'^h[2-9]$') # Паттерн для выделения всех элементов с тэгов h[2-9]

        clean_pattern = r'<[^>]*>' # Паттерн удаления HTML-тэгов.

        tags = [
            search_pattern, # Ищем со второго по 9
            'p',
            'ul'
        ]
    ...
    def parse_thematic(self,
                       n_news: int) -> list[dict]:
        '''
        ...
        while len(news_list) < n_news:
            ...
            try:
                ...
                hrefs = [link.get('href') for link in links] # Complete list of page href's

                # II. Parse all the news from the page

                for href in tqdm(hrefs,
                                total = len(hrefs),
                                desc = 'Data Gathering is in process...'):
                ...
                next_page_link = elements.find('a', class_= 'btn lenta_pager_next').get('href')
